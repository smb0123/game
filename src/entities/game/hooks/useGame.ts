...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case "SELECT_CARD": {
      ...
      const newRemainingSafeCards = card.isBomb
        ? state.remainingSafeCards
        : state.remainingSafeCards - 1;
      ...
      if (card.isBomb) {
        newStatus = "lost";
        // 게임이 끝났을 때 모든 카드를 보여주기
        finalCards = updatedCards.map((c) => ({ ...c, isRevealed: true }));
...
export function useGame() {
  ...
  const gameResult = useMemo((): GameResult | null => {
    ...
    const bombCards = state.cards
      .filter((card) => card.isBomb)
      .map((card) => card.id);
