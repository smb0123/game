...
// 게임 카드 Props 인터페이스
export interface GameCardProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'id' | 'onClick' | 'content'> {
  /** 카드 ID */
  id: string | number;
  /** 카드 상태 */
  state: GameCardState;
...
export const GameCard = React.forwardRef<HTMLDivElement, GameCardProps>(
  ({ className, id, state = 'hidden', clickable = true, onClick, content, size = 'md', ...props }, ref) => {
    // 카드 클릭 핸들러
    const handleClick = () => {
    ...
    return (
      <Card
        className={cn(
          ...
          state === 'selected' && 'ring-2 ring-blue-500 shadow-xl bg-yellow-100',
      ...
      >
        ...
        <div className="text-center">
          ...
          {state === 'safe' && <span className="text-green-700">✓</span>}
          {state === 'bomb' && <span className="text-red-700">💣</span>}
          {state === 'selected' && content && <span className="text-primary-700">{content}</span>}
        </div>
      </Card>
    );
  }
